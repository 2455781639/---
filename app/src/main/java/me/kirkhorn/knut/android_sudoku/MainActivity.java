...
public class MainActivity extends AppCompatActivity {
    ...
    private void checkCurrentLocale() {
        ...
        if (currentLanguage == null) {
            ...
            if (locale.getLanguage().equals("cn")) {
                //China is selected
                Log.i(TAG, "中文已设置");
                editor.putString("app_language", "cn");
                editor.apply();
                configuration.locale = new Locale("cn", "CN");
    ...
    private void refreshViewLanguages() {
        ...
        Button ranklist = findViewById(R.id.button6);
        ranklist.setText(R.string.ranking_list);
    ...
    public void onShowRanklist(View view) {
        Intent intent = new Intent("me.kirkhorn.knut.rank_listActivity");
        startActivity(intent);
    }

    public void onChinaFlagClick(View view) {
        if (currentEnglish) {
            Log.i(TAG, "Chinese is now selected");
            SharedPreferences sharedPreferences = getDefaultSharedPreferences(this);
            SharedPreferences.Editor editor = sharedPreferences.edit();
            Resources resources = getBaseContext().getResources();
            Configuration configuration = new Configuration();
            ...
            Toast.makeText(this, "中文已设置完成", Toast.LENGTH_SHORT).show();
        } else {
            Log.i(TAG, "中文已经设置");
            Toast.makeText(this, "中文已经设置", Toast.LENGTH_SHORT).show();
        }
    }
